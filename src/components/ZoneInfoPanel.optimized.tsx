...
/**
 * Zone 정보 패널 컴포넌트 (최적화 버전)
 * 화면 중앙에 표시되는 모달 형태의 업체 리스트 UI
 */
const ZoneInfoPanel = memo(function ZoneInfoPanel({ zoneId, onClose }: ZoneInfoPanelProps) {
  ...
  const { isMarkerClick, setSelectedZone, closeFullscreenCanvas } = useZoneStore()
  
  // Zone 설명 데이터 - useMemo로 메모이제이션
  const zoneDescriptions = useMemo<Record<string, ZoneDescription>>(() => ({
    'zone-1': {
      title: 'Zone 1',
  ...
  const handleClose = useCallback(() => {
    ...
    onClose() // Zone 선택 해제
  }, [closeFullscreenCanvas, onClose])
  
  const handleCompanyClick = useCallback((companyId: number, companyName: string) => {
    setSelectedZone(companyId.toString(), [0, 0, 0], false)
    handleClose()
    // URL 변경
    window.history.pushState({}, '', `/room/${companyId}`)
  }, [setSelectedZone, handleClose])
  
  const zoneInfo = zoneDescriptions[zoneId] || { 
    title: `Zone ${zoneId?.replace('zone-', '')}`, 
